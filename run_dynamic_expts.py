...
task_list = [{"task_id":'DE1', "n_out":1, "n_loc":1, "out_nonlin":'linear'},
             ...
             {"task_id":'GDE2',"n_out":1, "n_loc":2, "out_nonlin":'lienar'},
             {"task_id":'VDE1',"n_out":1, "n_loc":1, "max_delay":100, "out_nonlin":'linear'},
             {"task_id":'Harvey2012', "n_out":1, "sigtc":15.0, "stim_rate":1.0, "n_loc":1, "out_nonlin":'sigmoid'},
             {"task_id":'SINE', "n_out":1, "n_loc":1, "alpha":0.25, "out_nonlin":'linear'},
             {"task_id":'COMP', "n_out":1, "n_loc":1, "out_nonlin": 'sigmoid'}
             ]
...
ex_inp_vec = np.reshape(ex_inp_vec,(-1, generator.stim_dur + generator.delay_dur +
                                    generator.resp_dur, (ExptDict["task"]["n_loc"]+1) * generator.n_in))
...
torch.save({'all_params_list':model.state_dict(),
             ...
             'infloss_vec':np.asarray(infloss_vec)}, 'dynamic_sigma%f_lambda%f_rho%f_model%i_task%i.pt'%(offdiag_val, diag_val, wdecay_coeff, m_ind, t_ind))